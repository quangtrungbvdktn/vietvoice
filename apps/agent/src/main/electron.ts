...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("agent:pair", async (_event, input) => {
    const pairingInput = { ...input, deviceId: randomUUID() };
    let deviceToken = "";
    const result = await redeemPairingCode(pairingInput, {
      request: fetch,
      save: async (token) => { deviceToken = token; await persistPairing(token, { secretStorage, store }); },
    });
    await store.saveConnection({ apiUrl: pairingInput.apiUrl, deviceId: pairingInput.deviceId });
    await startPolling(pairingInput.apiUrl, deviceToken);
    return result;
  });
